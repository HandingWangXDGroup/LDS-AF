classdef BBOB51 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            PopObj(1,:) = benchmarks(PopDec',17);
            PopObj(2,:) = benchmarks(PopDec',20);
            PopObj = PopObj';
        end
        %% Generate a point for hypervolume calculation
        function R = GetOptimum(obj,N)
            R = [-38.720000000000000,1.831200000000000e+02];
        end
        %% Display a population in the objective space
        function DrawObj(obj,Population)
            Popobj(1,:) = benchmarks(Population.decs',17);
            Popobj(2,:) = benchmarks(Population.decs',20);
            Popobj = Popobj';
