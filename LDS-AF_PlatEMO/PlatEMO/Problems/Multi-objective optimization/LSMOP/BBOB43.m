classdef BBOB43 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            PopObj(1,:) = benchmarks(PopDec',14);
            PopObj(2,:) = benchmarks(PopDec',17);
            PopObj = PopObj';
        end
        %% Generate a point for hypervolume calculation
        function R = GetOptimum(obj,N)
            R = [-57.900000000000000,-38.720000000000000];
        end
        %% Display a population in the objective space
        function DrawObj(obj,Population)
            Popobj(1,:) = benchmarks(Population.decs',14);
            Popobj(2,:) = benchmarks(Population.decs',17);
            Popobj = Popobj';
