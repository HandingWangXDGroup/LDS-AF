classdef BBOB13 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            PopObj(1,:) = benchmarks(PopDec',2);
            PopObj(2,:) = benchmarks(PopDec',8);
            PopObj = PopObj';
        end
        %% Generate a point for hypervolume calculation
        function R = GetOptimum(obj,N)
            R = [2.763200000000000e+02,-1.351300000000000e+02];
        end
        %% Display a population in the objective space
        function DrawObj(obj,Population)
            Popobj(1,:) = benchmarks(Population.decs',2);
            Popobj(2,:) = benchmarks(Population.decs',8);
            Popobj = Popobj';
