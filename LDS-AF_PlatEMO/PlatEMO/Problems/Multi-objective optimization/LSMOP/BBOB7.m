classdef BBOB7 < PROBLEM
    ...
    methods
        ...
        function PopObj = CalObj(obj,PopDec)
            PopObj(1,:) = benchmarks(PopDec',1);
            PopObj(2,:) = benchmarks(PopDec',15);
            PopObj = PopObj';
        end
        %% Generate a point for hypervolume calculation
        function R = GetOptimum(obj,N)
            R = [-92.650000000000000,-44.770000000000000];
        end
        %% Display a population in the objective space
        function DrawObj(obj,Population)
            Popobj(1,:) = benchmarks(Population.decs',1);
            Popobj(2,:) = benchmarks(Population.decs',15);
            Popobj = Popobj';
